...
const CSS = `
.split-flap-text{display:inline-flex;align-items:center;gap:var(--split-flap-gap,6px);color:var(--split-flap-text-color,#f8fafc);font-family:var(--split-flap-font-family,'SFMono-Regular','Roboto Mono',Menlo,monospace);font-size:var(--split-flap-font-size,52px);font-weight:760;line-height:1;letter-spacing:.035em;white-space:pre;user-select:none;font-variant-numeric:tabular-nums}
...
if (typeof document !== 'undefined' && !document.getElementById('split-flap-text-css')) {
  const el = document.createElement('style');
  el.id = 'split-flap-text-css';
  el.textContent = CSS;
...
const SplitFlapText = ({
  ...
  const phrasesKey = typeof text === 'string' ? text : sourceWords.map(w => String(w ?? '')).join('\u001f');
  ...
  useEffect(() => {
    ...
    const animateTo = targetPhrase => {
      ...
      const plans = targetPhrase.split('').map((targetChar, index) => {
        const fromChar = fromPhrase[index] || ' ';
        if (fromChar === targetChar) return null;
        return {
          index, from: fromChar, target: targetChar,
          sequence: buildSequence(targetChar, safeFlips, activeCharset),
          start: index * safeStaggerMs, step: -1, done: false
        };
      }).filter(Boolean);
      ...
      const totalDuration = plans.reduce((max, p) => Math.max(max, p.start + p.sequence.length * safeFlipMs), 0);
      ...
      const updateTiles = updates => {
        setTiles(previous => {
          ...
          updates.forEach(u => {
            const tile = nextTiles[u.index];
            if (!tile) return;
            nextTiles[u.index] = { current: u.current, next: u.next, flipping: !u.done, tick: tile.tick + 1 };
          });
          return nextTiles;
        });
      };
  ...
  if (autoWidth) {
    let last = -1;
    tiles.forEach((t, i) => {
      const a = t.current, b = t.next;
      if ((a && a !== ' ') || (b && b !== ' ')) last = i;
    });
    visibleTiles = tiles.slice(0, last + 1);
  }
  ...
  return (
    <div className={`split-flap-text ${className}`.trim()} style={componentStyle} aria-label={settledText || undefined} {...props}>
      {visibleTiles.map((tile, index) => (
        <span className="split-flap-text__tile" aria-hidden="true" key={`${index}-${visibleTiles.length}`}>
